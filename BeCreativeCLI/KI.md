...
- Skripte: `table-script <tabelle> <aktion> [<skript>]` mit aktion = import|export|compare|execute|check|names. Die Skript-Dateien liegen im Ordner der Tabelle und heißen `<tabelle>.<skript>.txt` (import liest sie, export schreibt sie, compare/names vergleichen). `script-syntax [filter]` listet die Syntax aller Skript-Befehle auf.
...
- `bcr table-columnarrangement <tabelle> [<ansicht> [<spalten, mit | getrennt>]]` — Ansichten lesen oder setzen. Lesen (kein CLI-Recht nötig): ohne `<ansicht>` alle Ansichten als `Nr<TAB>Name<TAB>spalte1|spalte2|...` auflisten; mit `<ansicht>` nur die Spaltenliste dieser Ansicht als `spalte1|spalte2|...` ausgeben — direkt als `<spalten>`-Argument wiederverwendbar. Setzen (NUR mit dem CLI-Recht `Change column arrangement`; speichert): mit `<spalten>` die Spalten einer bestehenden Spaltenanordnung setzen, Reihenfolge = Reihenfolge der Angabe; nicht genannte Spalten werden ausgeblendet. `<ansicht>` = Nummer (ab 1, beim Lesen auch 0) oder Name; Ansicht 0 (alle Spalten) ist unveränderlich, neue Ansichten sind per CLI nicht möglich; Systemspalten und Doppel-Eintragungen werden abgelehnt
- `bcr table-script <tabelle> execute <skript> [--rowkey <key>]` — Tabellen-Skript ausführen (NUR mit dem CLI-Recht `Execute script`; Zeilen-Skripte brauchen `--rowkey`; speichert bei Änderungen)
- `bcr table-script <tabelle> import <skript>` — Inhalt der Datei `<tabelle>.<skript>.txt` aus dem Tabellen-Ordner in das Skript übernehmen (NUR mit dem CLI-Recht `Edit script`; legt das Skript an, falls neu; bei Syntax-Fehlern nichts ändernd)
- `bcr table-script <tabelle> export <skript>` — Skripttext in die Datei `<tabelle>.<skript>.txt` schreiben (NUR mit dem CLI-Recht `Edit script`)
- `bcr table-script <tabelle> compare <skript>` — Datei und Skript vergleichen; meldet `Identisch`/`Nicht identisch`, Exit-Code 1 bei Abweichung (NUR mit dem CLI-Recht `Edit script`)
- `bcr table-script <tabelle> check <skript>` — Fehlerprüfung des Skripts in der Tabelle; Exit-Code 1 bei Fehlern (NUR mit dem CLI-Recht `Edit script`)
