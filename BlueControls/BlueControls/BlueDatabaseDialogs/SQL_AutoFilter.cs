...
public partial class SQL_AutoFilter : FloatingForm //System.Windows.Forms.UserControl //
{
    #region Fields

    private readonly SQL_ColumnItem? _column;
    ...
    public SQL_AutoFilter(SQL_ColumnItem? column, SQL_FilterCollection? filter, List<SQL_RowItem>? pinned) : base(Design.Form_AutoFilter) {
    ...
    public event EventHandler<SQL_FilterComandEventArgs> FilterComand;

    #endregion

    #region Methods

    public static List<string> Autofilter_ItemList(SQL_ColumnItem column, SQL_FilterCollection? filter, List<SQL_RowItem?> pinned) {
        if (filter == null || filter.Count < 0) { return column.Contents(); }
        SQL_FilterCollection tfilter = new(column.Database);
    ...
    public void GenerateAll(SQL_FilterCollection? filter, List<SQL_RowItem?> pinned) {
        var nochOk = true;
        ...
        SQL_ColumnItem? lColumn = null;
        if (_column.Format == DataFormat.Verknüpfung_zu_anderer_Datenbank) {
            (lColumn, _, _) = SQL_CellCollection.LinkedCellData(_column, null, false, false);
        }
        if (lColumn == null) { lColumn = _column; }

        Width = Math.Max(txbEingabe.Width + (Skin.Padding * 2), SQL_Table.TmpColumnContentWidth(null, lColumn, f, 16));
        ...
        lsbFilterItems.Width = Math.Min(SQL_Table.TmpColumnContentWidth(null, lColumn, f, 16), Width - (Skin.PaddingSmal * 2));
        ...
        if (lColumn.FilterOptions is not FilterOptions.Enabled_OnlyAndAllowed and not FilterOptions.Enabled_OnlyOrAllowed) {
            ...
            var tmp = SQL_CellItem.ValueReadable(lColumn, string.Empty, ShortenStyle.Replaced, BildTextVerhalten.Nur_Text, true);
    ...
    private void butFertig_Click(object sender, System.EventArgs e) {
        ...
        if (_multiAuswahlOder) {
            CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Istgleich_ODER_GroßKleinEgal, searchValue));
            return;
        }
        if (_multiAuswahlUnd) {
            CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Istgleich_UND_GroßKleinEgal, searchValue));
            return;
        }
    ...
    private void CloseAndDispose(string comand, SQL_FilterItem? newFilter) {
        if (IsClosed) { return; }
        Close();
        OnFilterComand(new SQL_FilterComandEventArgs(comand, _column, newFilter));
    }
    ...
    private void FiltItems_ItemClicked(object sender, BasicListItemEventArgs e) {
        ...
        if (_negativAuswahl) {
            // Nur Ohne Multirow
            CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Ungleich_MultiRowIgnorieren_GroßKleinEgal, l));
        } else {
            CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Istgleich_ODER_GroßKleinEgal, l));
        }
    }

    private void OnFilterComand(SQL_FilterComandEventArgs e) => FilterComand?.Invoke(this, e);

    private void sFilter_ItemClicked(object sender, BasicListItemEventArgs e) {
        switch (e.Item.Internal.ToLower()) {
            case "filterleere": {
                    CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Istgleich | FilterType.MultiRowIgnorieren, ""));
                    break;
                }

            case "filternichtleere": {
                    CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Ungleich_MultiRowIgnorieren, ""));
                    break;
                }
            ...
            if (txbEingabe.Text.Contains("-")) {
                ...
                if (l > 0 && l < tmp.Length - 1) {
                    ...
                    if (z1.IsDouble() && z2.IsDouble()) {
                        ...
                        CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Between | FilterType.UND, zd1.ToString(Constants.Format_Float9) + "|" + zd2.ToString(Constants.Format_Float9)));
                        return;
                    }
                }
            }
        }
        CloseAndDispose("Filter", new SQL_FilterItem(_column, FilterType.Instr | FilterType.GroßKleinEgal, txbEingabe.Text));
    }
