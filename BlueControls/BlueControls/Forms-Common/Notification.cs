...
namespace BlueControls.DialogBoxes
{
    public partial class Notification : FloatingForm
    {
        ...
        public new void Close()
        {
            ...
            {
                if (!ThisForm.IsDisposed && ThisForm is Notification Note)
                {
                    try
                    {
                        base.Close();
        ...
        private void timNote_Tick(object sender, System.EventArgs e)
        {
            ...
            if (FloatInAndOutMilliSek > 0)
            {
                ...
                Region = new Region(new Rectangle(0, 0, Width, Convert.ToInt32(Math.Truncate(Height * _Proz))));
                Top = Convert.ToInt32(Math.Truncate(System.Windows.Forms.Screen.PrimaryScreen.WorkingArea.Bottom - Height * _Proz));
                Opacity = _Proz;
