...
public partial class SQL_TableView : Form {
    ...
    private SQL_Database? _originalDb;

    #endregion

    #region Constructors

    public SQL_TableView() : this(null, true, true) { }

    public SQL_TableView(Database? database, bool loadTabVisible, bool adminTabVisible) {
        InitializeComponent();
    ...
    public static void CheckDatabase(object? sender, LoadedEventArgs? e) {
        if (sender is SQL_Database database && !database.ReadOnly) {
    ...
    protected virtual void DatabaseSet(SQL_Database? database, string toParse) {
    ...
    protected override void OnFormClosing(System.Windows.Forms.FormClosingEventArgs e) {
        DatabaseSet(null as SQL_Database, string.Empty);
    ...
    protected virtual void TableView_ContextMenuItemClicked(object sender, ContextMenuItemClickedEventArgs e) {
        ...
        var editable = string.IsNullOrEmpty(SQL_CellCollection.ErrorReason(column, row, ErrorReason.EditAcut));
        ...
        switch (e.ClickedComand) {
            ...
            case "SpaltenSortierungAZ":
                tbl.SortDefinitionTemporary = new SQL_RowSortDefinition(tbl.Database, column.Name, false);
                break;

            case "SpaltenSortierungZA":
                tbl.SortDefinitionTemporary = new SQL_RowSortDefinition(tbl.Database, column.Name, true);
                break;
            ...
            case "ContentCopy":
                SQL_Table.CopyToClipboard(column, row, true);
                break;
    ...
    private void btnSuchenUndErsetzen_Click(object sender, System.EventArgs e) {
    }
    ...
    private void ChangeDatabase(SQL_Database? database) {
        if (_originalDb != null) {
    ...
    private string NameRepair(string istName, SQL_RowItem? vRow) {
        var newName = istName;
        var istZ = 0;
        do {
            var changed = false;
    ...
    private void SuchEintragNoSave(Direction richtung, out SQL_ColumnItem? column, out SQL_RowData? row) {
    ...
    private void Table_EditBeforeBeginEdit(object sender, SQL_CellCancelEventArgs e) {
    ...
    private void Table_SelectedCellChanged(object sender, SQL_CellExtEventArgs e) {
        if (InvokeRequired) {
        ...
        if (ckbZeilenclickInsClipboard.Checked) {
            SQL_Table.CopyToClipboard(e.Column, e.RowData.Row, false);
            Table.Focus();
        }
    }

    private void Table_SelectedRowChanged(object sender, SQL_RowEventArgs? e) {
        if (InvokeRequired) {
    ...
    private void Table_ViewChanged(object sender, System.EventArgs e) => SQL_Table.WriteColumnArrangementsInto(cbxColumnArr, Table.Database, Table.Arrangement);
    ...
    private void TableView_DatabaseChanged(object sender, System.EventArgs e) {
        SQL_Table.WriteColumnArrangementsInto(cbxColumnArr, Table.Database, Table.Arrangement);
