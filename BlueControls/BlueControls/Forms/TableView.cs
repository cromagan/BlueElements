...
public partial class TableView : FormWithStatusBar {
    ...
    protected virtual void TableView_ContextMenu_Init(object sender, ContextMenuInitEventArgs e) {
        ...
        _ = e.UserMenu.Add("Auf Fehler prüfen", "Datenüberprüfung", QuickImage.Get(ImageCode.HäkchenDoppelt, 16), row != null && tbl.Database.isRowScriptPossible(true));

        foreach (var thiss in tbl.Database.EventScript) {
            if (thiss != null && thiss.ManualExecutable && thiss.NeedRow) {
                _ = e.UserMenu.Add("Skript: " + thiss.ReadableText(), "Skript|" + thiss.KeyName, ImageCode.Skript, row != null && tbl.Database.isRowScriptPossible(true));
            }
        }
    }
