...
internal sealed partial class SearchAndReplaceInCells : Form {
    ...
    public SearchAndReplaceInCells(Controls.TableView table) {
    ...
    private void Checkbuttons() {
        var canDo = true;
        if (IsDisposed || _table.Table is not { IsDisposed: false } db) { return; }
    ...
    private void ers_Click(object sender, System.EventArgs e) {
        ...
        foreach (var thisRow in db.Row) {
            if (!chkAktuelleFilterung.Checked || thisRow.MatchesTo([.. _table.FilterCombined]) || _table.PinnedRows.Contains(thisRow)) {
