...
internal class HrChar : Char {

    #region Constructors

    public HrChar() { }

    internal HrChar(ExtText parent, List<string> overrideTags) : base(parent, overrideTags) { }
    ...
    internal override (float ContinueX, float ContinueY, float MaxRight, float MaxBottom) ComputeCharLayout(float startX, float startY, float maxWidth, float lineStartX, float lineSpacing) {
        var h = Font is null ? 8f : Font.CharHeight;
    ...
    protected override SizeF CalculateSizeCanvas() => Font is null ? new SizeF(0, 8) : new SizeF(0, Font.CharHeight);
