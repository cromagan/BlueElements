...
internal sealed partial class ColumnEditor : IIsEditor, IHasTable {
    ...
    private void Column_DatenAuslesen() {
        ...
        chkMultiline.SetChecked(() => c.MultiLine);
        chkIsFirst.SetChecked(() => c.IsFirst);
        chkIsKeyColumn.SetChecked(() => c.IsKeyColumn);
        chkRelation.SetChecked(() => c.Relationship_to_First);
        cbxRelationType.Set(((int)c.RelationType).ToString1(), () => c.RelationType);
        cbxChunk.Set(((int)c.Value_for_Chunk).ToString1(), () => c.Value_for_Chunk);
        cbxRandLinks.Set(((int)c.LineStyleLeft).ToString1(), () => c.LineStyleLeft);
        cbxRandRechts.Set(((int)c.LineStyleRight).ToString1(), () => c.LineStyleRight);
        cbxAlign.Set(((int)c.Align).ToString1(), () => c.Align);
        cbxBackground.Set(((long)c.BackgroundStyle).ToString1(), () => c.BackgroundStyle);
        cbxAdditionalCheck.Set(((int)c.AdditionalFormatCheck).ToString1(), () => c.AdditionalFormatCheck);
        cbxScriptType.Set(((int)c.ScriptType).ToString1(), () => c.ScriptType);
        cbxTranslate.Set(((int)c.DoOpticalTranslation).ToString1(), () => c.DoOpticalTranslation);
        cbxRenderer.Set(() => c.DefaultRenderer);
        cbxRenderer_TextChanged(cbxRenderer, System.EventArgs.Empty);
        cbxSort.Set(((int)c.SortType).ToString1(), () => c.SortType);
        btnAutoFilterMoeglich.SetChecked(() => c.FilterOptions.HasFlag(FilterOptions.Enabled));
        btnAutoFilterTXTErlaubt.SetChecked(() => c.FilterOptions.HasFlag(FilterOptions.TextFilterEnabled));
        btnAutoFilterErweitertErlaubt.SetChecked(() => c.FilterOptions.HasFlag(FilterOptions.ExtendedFilterEnabled));
        chkFilterOnlyOr.SetChecked(() => c.FilterOptions.HasFlag(FilterOptions.OnlyOrAllowed));
        chkFilterOnlyAND.SetChecked(() => c.FilterOptions.HasFlag(FilterOptions.OnlyAndAllowed));
        btnZeilenFilterIgnorieren.SetChecked(() => c.IgnoreAtRowFilter);
        cbxControlStrategy.Set(() => c.ControlStrategy);
        cbxControlStrategy_TextChanged(cbxControlStrategy, System.EventArgs.Empty);
        btnEditableStandard.SetChecked(() => c.EditableWithTextInput);
        txbMinTextLength.Set(() => c.MinTextLength);
        btnAutoEditAutoSort.SetChecked(() => c.AfterEditQuickSortRemoveDouble);
        txbRunden.Set(c.AfterEditRound is > -1 and < 7 ? c.AfterEditRound.ToString1() : string.Empty, () => c.AfterEditRound);
        txbFixedColumnWidth.Set(c.FixedColumnWidth > 0 ? c.FixedColumnWidth.ToString1() : string.Empty, () => c.FixedColumnWidth);
        btnAutoEditToUpper.SetChecked(() => c.AfterEditDoUCase);
        btnAutoEditKleineFehler.SetChecked(() => c.AfterEditAutoCorrect);
        txbJoker.Set(() => c.AutoFilterJoker);
        txbUeberschift1.Set(() => c.CaptionGroup1);
        txbUeberschift2.Set(() => c.CaptionGroup2);
        txbUeberschift3.Set(() => c.CaptionGroup3);
        txbSpaltenbild.Set(() => c.CaptionBitmapCode);
        chkSaveContent.SetChecked(() => c.SaveContent);
        txbAuswaehlbareWerte.Set(string.Join('\r', c.DropDownItems), () => c.DropDownItems);
        txbAutoReplace.Set(string.Join('\r', c.AfterEditAutoReplace), () => c.AfterEditAutoReplace);
        txbRegex.Set(() => c.RegexCheck);
        txbTags.Set(string.Join('\r', c.ColumnTags), () => c.ColumnTags);
        lbxCellEditor.Check(c.PermissionGroupsChangeCell, true);
        txbAllowedChars.Set(c.AllowedChars.EncodeControlChars(), () => c.AllowedChars);
        txbMaxTextLength.Set(() => c.MaxTextLength);
        txbMaxCellLength.Set(() => c.MaxCellLength);
        btnOtherValuesToo.SetChecked(() => c.ShowValuesOfOtherCellsInDropdown);
        btnIgnoreLock.SetChecked(() => c.EditAllowedDespiteLock);
        txbAdminInfo.Set(c.AdminInfo.Replace("<br>", "\r", RegexOptions.IgnoreCase), () => c.AdminInfo);
        txbQuickinfo.Set(c.QuickInfo.Replace("<br>", "\r", RegexOptions.IgnoreCase), () => c.QuickInfo);
        cbxLinkedTable.Set(() => c.LinkedTableTableName);
        txbAutoRemove.Set(c.ForbiddenChars.EncodeControlChars(), () => c.ForbiddenChars);
