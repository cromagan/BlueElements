...
public static class ListItemExtension {
    ...
    public static List<ListItem> AllAvailableTables() {
        var ld = Table.AllAvailableTables();
        var ld2 = new List<ListItem>();
    ...
    /// </summary>
    /// <returns></returns>
    public static (int BiggestItemX, int BiggestItemY, int HeightAdded, Orientation Orientation) CanvasItemData(this IEnumerable<ListItem> item, Design itemDesign) {
        try {
            var w = 16;
            var h = 0;
            var hall = 0;
    ...
    public static void DrawItems(this IEnumerable<ListItem>? list, Graphics gr, Rectangle visControlArea, ListItem? _mouseOverItem, int offsetX, int offsetY, string FilterText, States controlState, Design _controlDesign, Design _itemDesign, Design checkboxDesign, List<string>? _checked, float zoom) {
    ...
    public static ListItem? ElementAtPosition(this List<ListItem>? list, int controlX, int controlY, float zoom, float offsetX, float offsetY) {
    ...
    /// <example>
    /// <code>
    /// var items = new List&lt;ListItem&gt; { item1, item2, item3 };
    /// var firstButton = items.First&lt;ButtonItem&gt;();
    /// </code>
    /// </example>
    public static T? First<T>(this List<ListItem>? list) where T : ListItem? {
        if (list is not { Count: > 0 }) { return null; }
    ...
    public static CellDesignListItem ItemOf(string value, ColumnItem columnStyle, Renderer.Renderer cellRenderer) => new(value, cellRenderer, true, columnStyle.DoOpticalTranslation, (Alignment)columnStyle.Align, columnStyle.SortType);

    /// <summary>
    /// Als Interner Name wird der RowKey als String abgelegt
    /// </summary>
    public static RowLayoutListItem ItemOf(RowItem row, string layoutId, string userDefCompareKey) => new(row, layoutId, userDefCompareKey);

    /// <summary>
    /// Als Interner Name wird der RowKey als String abgelegt
    /// </summary>
    public static RowLayoutListItem ItemOf(RowItem row, string layoutId) => ItemOf(row, layoutId, string.Empty);
    ...
    public static List<ListItem> ItemsOf(IEnumerable<ColumnItem> columns) {
        ...
        foreach (var thisColumnItem in columns) {
            if (thisColumnItem is not null) {
                ...
                co.UserDefCompareKey = capt + SecondSortChar + thisColumnItem.KeyName;

                if (!cl.Contains(capt)) {
                    cl.Add(capt);
                    l.Add(new TextListItem(capt, capt, null, true, true, string.Empty, capt + FirstSortChar));
                }
    ...
    public static List<ListItem> ItemsOf(IEnumerable<string>? list) {
        var l = new List<ListItem>();
    ...
    public static List<ListItem> ItemsOf(ICollection<string>? values, ColumnItem columnStyle, Renderer.Renderer renderer) {
        var l = new List<ListItem>();
...
public abstract class ListItem : IComparable, IHasKeyName, IHasQuickInfo, INotifyPropertyChanged, IDisposableExtended {
    ...
    protected ListItem(string keyName, bool enabled) {
