...
public sealed class ZoneMarkRenderer {

    #region Constructors

    public ZoneMarkRenderer(MarkRenderer renderer, int startPos, int endPos, Color? fontColor) {
        _renderer = renderer;
        StartPos = startPos;
        EndPos = endPos;
        FontColor = fontColor;
    ...
    private void DrawOverriddenText(ExtText extText, Graphics gr, float zoom, int offsetX, int offsetY, Color overrideColor) {
        ...
        for (var i = markStart; i <= markEnd; i++) {
            ...
            if (Char.IsVisible(extText.AreaControl, controlPos, controlSize)) {
                var f = t.Font;
                if (f is not null) {
