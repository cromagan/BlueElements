...
public abstract class PadItem : ParseableItem, IReadableTextWithKey, IMoveable, IComparable, ISimpleEditor, IJsonParseable {
    ...
    public static readonly AssemblyAwareCache<PadItem> AllPadItems = new();
    ...
    protected PadItem(string keyName) : base() {
        BeginInit();
    ...
    public bool ShowAlways {
        get {
            if (Parent is CollectionPadItem { IsDisposed: false } icpi) { return icpi.ShowAlways; }
            return this is CollectionPadItem { IsDisposed: false, ShowAlways: true }; // Wichtig, wegen NEW!
        }
    }
    ...
    public virtual void AddedToCollection(CollectionPadItem parent) {
        if (IsDisposed) { return; }
        Parent = parent;
    }
