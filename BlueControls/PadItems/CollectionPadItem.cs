...
public sealed class CollectionPadItem : SizeableRectanglePadItem, IEnumerable<PadItem>, IReadableTextWithKey, IParseable, IJsonParseable, ICanHaveVariables, IStyleable {
    ...
    private readonly ObservableCollection<PadItem> _internal = [];
    private readonly object _itemLock = new();

    private List<ScriptVariable>? _cachedExportVariables;
    ...
    public CollectionPadItem() : base(string.Empty) {
    ...
    public CollectionPadItem(string layoutFileName) : this() {
    ...
    public PadItem? this[string keyName] => _internal.GetByKey(keyName);

    public PadItem? this[int nr] => _internal[nr];
    ...
    public static List<(IAutosizable item, RectangleF newpos)> ResizeControls(CollectionPadItem padData, float newWidthPixel, float newhHeightPixel, string mode) {
        ...
        foreach (var thisc in padData._internal) {
            ...
            if (IsInDrawingArea(thisc.CanvasUsedArea, padData.CanvasUsedArea.ToRect())) {
                its.Add(aas);
            } else if (thisc is ReciverPadItem { MustBeInDrawingArea: false }) {
    ...
    public void BringToFront(PadItem thisItem) {
    ...
    public bool Contains(PadItem item) => _internal.Contains(item);

    public void EineEbeneNachHinten(PadItem bpi) {
        var i2 = Previous(bpi);
        if (i2 is not null) {
    ...
    public void EineEbeneNachVorne(PadItem bpi) {
        var i2 = Next(bpi);
        if (i2 is not null) {
            Swap(_internal.IndexOf(bpi), _internal.IndexOf(i2));
        }
    ...
    public Bitmap? GeneratePreviewBitmap(int maxSize) {
        ...
        var zoom = Math.Min(maxSize / usedArea.Width, maxSize / usedArea.Height);
    ...
    public IEnumerator<PadItem> GetEnumerator() => ((IEnumerable<PadItem>)_internal).GetEnumerator();
    ...
    public PadItem? HotItem(Point controlPoint, bool topLevel, bool mustEnabled, float zoom, float offsetX, float offsetY) {
        ...
        if (smallestHotItem is CollectionPadItem { IsDisposed: false } icpi) {
    ...
    public override void ParseJson(JsonObject json) {
        ...
        try {
            ...
            foreach (var created in json.GetList<PadItem>("items", false)) {
                Add(created);
            }
    ...
    public override bool ParseThis(string key, string value) {
        switch (key) {
            ...
            case "item":
                var i = NewByParsing<PadItem>(value.FromNonCritical());
    ...
    public void Remove(PadItem? item) {
        if (IsDisposed) { return; }
    ...
    public void RemoveRange(List<PadItem> remove) {
    ...
    //Used: Only BZL
    public bool ReplaceVariable(string name, string wert) => ReplaceVariable(new StringScriptVariable(name, wert));
    ...
    public void SendToBack(PadItem thisItem) {
    ...
    public List<string> VisibleFor_AllUsed() {
        ...
        foreach (var thisIt in _internal) {
            if (thisIt is ReciverPadItem csi) {
                l.AddRange(csi.VisibleFor);
            }

            if (thisIt is CollectionPadItem { IsDisposed: false } icpi) {
    ...
    internal ConnectedFormula? GetConnectedFormula() {
        ...
        if (Parent is CollectionPadItem icpi) { return icpi.GetConnectedFormula(); }
        return null;
    }
    ...
    internal override void GetNewIdsForEverything() {
        ...
        foreach (var child in new List<PadItem>(_internal)) {
    ...
    internal bool HasVisibleItemsForMe(string mode) {
        ...
        foreach (var thisItem in this) {
            if (thisItem is ReciverPadItem { MustBeInDrawingArea: true } cspi) {
    ...
    internal PadItem? Next(PadItem bpi) {
    ...
    internal PadItem? Previous(PadItem bpi) {
        var itemCount = _internal.IndexOf(bpi);
        if (itemCount < 0) { Develop.DebugError("Item im SortDefinition nicht enthalten"); }
        do {
            itemCount--;
    ...
    protected override void DrawExplicit(Graphics gr, Rectangle visibleAreaControl, RectangleF positionControl, float zoom, float offsetX, float offsetY, bool forPrinting) => DrawItems(gr, visibleAreaControl, positionControl, zoom, offsetX, offsetY, forPrinting, AutoZoomFit);
    ...
    private bool CreateItems(string toParse) {
        ...
        foreach (var pair in x) {
            switch (pair.Key.ToLowerInvariant()) {
                ...
                case "item":
                    var i = NewByParsing<PadItem>(pair.Value);
                    if (i is not null) { Add(i); }
                    break;
    ...
    private void OnItemAdded() {
        ...
        // PadItem.OnPropertyChanged), damit nach einem Parse kein
