...
public class TabFormulaPadItem : ReciverPadItem, IItemToControl, IAutosizable {
    ...
    public TabFormulaPadItem(string keyName, ConnectedFormula? cformula) : base(keyName, cformula) => ParentFormula?.PropertyChanged += ParentFormula_PropertyChanged;
    ...
    public List<CollectionPadItem> ListOfChildsPages() {
        var tmp = new List<CollectionPadItem>();
    ...
    private Controls.ListBox Childs() {
        ...
        var notUse = new List<string>(tmp.ToListOfString());
        if (ParentFormula?.NotAllowedChilds is { } nc) { notUse.AddRange(nc); }
    ...
    private void Childs_ItemCheckedChanged(object? sender, System.EventArgs e) {
        ...
        foreach (var thisIt in lb.Items) {
            if (thisIt is ReadableListItem rli && rli.Item is CollectionPadItem icpi) {
                if (icpi.IsHead()) {
