...
public class TextPadItem : SizeableRectanglePadItem, ICanHaveVariables, IStyleableOne, ISupportsTextScale {
    ...
    public override List<GenericControl> GetProperties(int widthOfControl) {
        ...
        var textFlex = new FlexiControlForProperty<string>(() => TextValue, 5);
    ...
    public override void ParseJson(JsonObject json) {
        BeginInit();
        try {
            TextValue = json.GetString("text", TextValue);
    ...
    public override bool ParseThis(string key, string value) {
        switch (key) {
            case "readabletext":
                TextValue = value.FromNonCritical().UnEscapeUnicode();
                return true;
    ...
    public bool ResetVariables() {
        if (IsDisposed) { return false; }
        if (TextValue == _textReplaced) { return false; }
        _textReplaced = TextValue;
        InvalidateText();
        OnPropertyChanged("Variables");
        return true;
    }
