...
public abstract class Renderer : ParseableItem, IReadableText, IHasQuickInfo, ISimpleEditor {
    ...
    public static readonly AssemblyAwareCache<Renderer> AllRenderers = new();

    internal static readonly Renderer Bool = new ImageAndTextRenderer("+|Häkchen\r\no|Kreis2\r\n-|Kreuz");
    internal static readonly Renderer Default = new ImageAndTextRenderer();
    ...
    protected Renderer() : this(false) { }

    protected Renderer(bool readOnly) => ReadOnly = readOnly;
    ...
    /// RichTextRenderer) berechnen die korrekte Höhe basierend auf
    ...
    /// CalculateContentSize. Mehrzeilige Renderer überschreiben dies,
    /// um die Höhe abhängig vom Zeilenumbruch zu berechnen.
    /// </summary>
    protected virtual Size CalculateContentSizeAtWidth(string content, TranslationType doOpticalTranslation, int canvasWidth) => CalculateContentSize(content, doOpticalTranslation);
    ...
    protected BlueFont GetFont(float additionalScale) => Math.Abs(1 - additionalScale) < DefaultTolerance ? GetFont() : GetFont().Scale(additionalScale);
