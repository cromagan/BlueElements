...
public sealed class SQL_CellCollection : Dictionary<string, SQL_CellItem>, IDisposable {
    ...
    public SQL_CellCollection(SQL_Database database) : base() {
        _database = database;
    ...
    public static string AutomaticInitalValue(SQL_ColumnItem? column, SQL_RowItem? row) {
        ...
        SQL_FilterCollection f = new(column.Database)
        {
            new SQL_FilterItem(cc, FilterType.Istgleich_GroßKleinEgal, row.CellGetString(cc)),
            new SQL_FilterItem(column, FilterType.Ungleich_MultiRowIgnorieren, string.Empty)
        };
    ...
    /// <returns></returns>
    public static string ErrorReason(SQL_ColumnItem? column, SQL_RowItem? row, ErrorReason mode) {
        ...
        if (SQL_ColumnItem.UserEditDialogTypeInTable(column.Format, false, true, column.MultiLine) == EditTypeTable.None) {
    ...
    public static (List<SQL_FilterItem> filter, string info) GetFilterFromLinkedCellData(SQL_Database linkedDatabase, SQL_ColumnItem column, SQL_RowItem? row) {
        var fi = new List<SQL_FilterItem>();
        ...
        for (var z = 0; z < Math.Min(column.LinkedCellFilter.Count, linkedDatabase.Column.Count); z++) {
            if (IntTryParse(column.LinkedCellFilter[z], out var key)) {
                ...
                fi.Add(new SQL_FilterItem(linkedDatabase.Column[z], FilterType.Istgleich, value));
            } else if (!string.IsNullOrEmpty(column.LinkedCellFilter[z]) && column.LinkedCellFilter[z].StartsWith("@")) {
                fi.Add(new SQL_FilterItem(linkedDatabase.Column[z], FilterType.Istgleich, column.LinkedCellFilter[z].Substring(1)));
            }
        }
    ...
    public static string KeyOfCell(SQL_ColumnItem? column, SQL_RowItem? row) {
    ...
    public static (SQL_ColumnItem? column, SQL_RowItem? row, string info) LinkedCellData(string column, SQL_RowItem? row, bool repairLinkedValue, bool addRowIfNotExists) {
    ...
    /// <returns></returns>
    public static (SQL_ColumnItem? column, SQL_RowItem? row, string info) LinkedCellData(SQL_ColumnItem? column, SQL_RowItem? row, bool repairLinkedValue, bool addRowIfNotExists) {
        if (column == null) { return (null, null, "Interner Spaltenfehler."); }

        if (column.Format is not DataFormat.Verknüpfung_zu_anderer_Datenbank) { return ((SQL_ColumnItem?)null, (SQL_RowItem?)null, "Format ist nicht LinkedCell."); }
    ...
    /// <returns></returns>
    public static bool UserEditPossible(SQL_ColumnItem? column, SQL_RowItem? row, ErrorReason mode) => string.IsNullOrEmpty(ErrorReason(column, row, mode));

    public void DataOfCellKey(string cellKey, out SQL_ColumnItem? column, out SQL_RowItem? row) {
    ...
    public void Delete(SQL_ColumnItem column, long rowKey) {
    ...
    public void DoSpecialFormats(SQL_ColumnItem? column, SQL_RowItem? row, string previewsValue, bool doAlways) {
        ...
        if (column.IsFirst()) {
            foreach (var thisSQL_ColumnItem in _database.Column.Where(thisSQL_ColumnItem => thisSQL_ColumnItem != null)) {
                switch (thisSQL_ColumnItem.Format) {
                    //case DataFormat.Relation:
                    //    RelationNameChanged(ThisSQL_ColumnItem, PreviewsValue, CurrentValue);
                    //    break;
                    case DataFormat.RelationText:
                        RelationTextNameChanged(thisSQL_ColumnItem, row.Key, previewsValue, currentValue);
                        break;
                }
            }
        }
    ...
    public bool GetBoolean(string columnName, SQL_RowItem? row) => GetBoolean(_database?.Column[columnName], row);

    public bool GetBoolean(SQL_ColumnItem? column, SQL_RowItem? row) => GetString(column, row).FromPlusMinus();// Main Method

    public Color GetColor(string columnName, SQL_RowItem? row) => GetColor(_database?.Column[columnName], row);

    public Color GetColor(SQL_ColumnItem? column, SQL_RowItem? row) => Color.FromArgb(GetInteger(column, row)); // Main Method

    public int GetColorBgr(SQL_ColumnItem? column, SQL_RowItem? row) {
    ...
    public DateTime GetDateTime(string columnName, SQL_RowItem? row) => GetDateTime(_database?.Column[columnName], row);

    public DateTime GetDateTime(SQL_ColumnItem? column, SQL_RowItem? row) // Main Method
    {
        var @string = GetString(column, row);
        return string.IsNullOrEmpty(@string) ? default : DateTimeTryParse(@string, out var d) ? d : default;
    }

    public double GetDouble(string columnName, SQL_RowItem? row) => GetDouble(_database?.Column[columnName], row);

    public double GetDouble(SQL_ColumnItem? column, SQL_RowItem? row) // Main Method
    {
        var x = GetString(column, row);
        return string.IsNullOrEmpty(x) ? 0 : DoubleParse(x);
    }

    public int GetInteger(string columnName, SQL_RowItem? row) => GetInteger(_database?.Column[columnName], row);

    public int GetInteger(SQL_ColumnItem? column, SQL_RowItem? row) // Main Method
    {
        var x = GetString(column, row);
        return string.IsNullOrEmpty(x) ? 0 : IntParse(x);
    }

    public List<string> GetList(string columnName, SQL_RowItem? row) => GetList(_database?.Column[columnName], row);

    public List<string> GetList(SQL_ColumnItem? column, SQL_RowItem? row) => GetString(column, row).SplitAndCutByCrToList();// Main Method

    public Point GetPoint(string columnName, SQL_RowItem? row) => GetPoint(_database?.Column[columnName], row);

    public Point GetPoint(SQL_ColumnItem? column, SQL_RowItem? row) // Main Method
    {
        var @string = GetString(column, row);
        return string.IsNullOrEmpty(@string) ? Point.Empty : @string.PointParse();
    }

    public Size GetSizeOfCellContent(SQL_ColumnItem column, SQL_RowItem row) {
    ...
    public string GetString(string columnName, SQL_RowItem? row) => GetString(_database?.Column[columnName], row);
    ...
    public string GetStringBehindLinkedValue(SQL_ColumnItem? column, SQL_RowItem? row) {
        if (column == null || row == null) { return string.Empty; }
        var cellKey = KeyOfCell(column, row);
    ...
    public bool IsNullOrEmpty(SQL_ColumnItem? column, SQL_RowItem? row) {
    ...
    public bool IsNullOrEmpty(string columnName, SQL_RowItem? row) => IsNullOrEmpty(_database.Column[columnName], row);
    ...
    public bool MatchesTo(SQL_ColumnItem? column, SQL_RowItem? row, SQL_FilterItem filter) {
        ...
        try {
            ...
            var fColumn = column;
            if (column.Format is DataFormat.Verknüpfung_zu_anderer_Datenbank) {
                var (SQL_ColumnItem, SQL_RowItem, _) = LinkedCellData(column, row, false, false);
                if (SQL_ColumnItem != null && SQL_RowItem != null) {
                    @string = SQL_RowItem.CellGetString(SQL_ColumnItem);
                    fColumn = SQL_ColumnItem;
                }
            } else {
                var cellKey = KeyOfCell(column, row);
    ...
    public void Set(string columnName, SQL_RowItem? row, string value) => Set(_database?.Column[columnName], row, value);
    ...
    public void Set(string columnName, SQL_RowItem? row, bool value) => Set(_database?.Column[columnName], row, value.ToPlusMinus());

    public void Set(SQL_ColumnItem? column, SQL_RowItem? row, bool value) => Set(column, row, value.ToPlusMinus());

    public void Set(string columnName, SQL_RowItem? row, DateTime value) => Set(_database?.Column[columnName], row, value.ToString(Constants.Format_Date5));

    public void Set(SQL_ColumnItem? column, SQL_RowItem? row, DateTime value) => Set(column, row, value.ToString(Constants.Format_Date5));

    public void Set(string columnName, SQL_RowItem? row, List<string>? value) => Set(_database?.Column[columnName], row, value);

    public void Set(SQL_ColumnItem? column, SQL_RowItem? row, List<string>? value) => Set(column, row, value.JoinWithCr());

    public void Set(string columnName, SQL_RowItem? row, Point value) => Set(_database?.Column[columnName], row, value);

    public void Set(SQL_ColumnItem? column, SQL_RowItem? row, Point value) => Set(column, row, value.ToString());      // Main Method// {X=253,Y=194} MUSS ES SEIN, prüfen

    public void Set(string columnName, SQL_RowItem? row, int value) => Set(_database?.Column[columnName], row, value.ToString());

    public void Set(SQL_ColumnItem? column, SQL_RowItem? row, int value) => Set(column, row, value.ToString());

    public void Set(string columnName, SQL_RowItem? row, double value) => Set(_database?.Column[columnName], row, value.ToString(Constants.Format_Float1));

    public void Set(SQL_ColumnItem? column, SQL_RowItem? row, double value) => Set(column, row, value.ToString(Constants.Format_Float1));
    ...
    public void SetSizeOfCellContent(SQL_ColumnItem? column, SQL_RowItem? row, Size contentSize) {
        var cellKey = KeyOfCell(column, row);
        if (!ContainsKey(cellKey)) { return; }
        this[cellKey].Size = contentSize;
    }

    public List<string> ValuesReadable(SQL_ColumnItem? column, SQL_RowItem row, ShortenStyle style) => SQL_CellItem.ValuesReadable(column, row, style);

    internal static List<SQL_RowItem?> ConnectedRowsOfRelations(string completeRelationText, SQL_RowItem? row) {
        List<SQL_RowItem?> allRows = new();
    ...
    internal static void Invalidate_CellContentSize(SQL_ColumnItem? column, SQL_RowItem? row) {
        var cellKey = KeyOfCell(column, row);
    ...
    internal string CompareKey(SQL_ColumnItem? column, SQL_RowItem? row) => GetString(column, row).CompareKey(column.SortType);

    internal Size ContentSizeToSave(KeyValuePair<string, CellItem> vCell, SQL_ColumnItem column) {
    ...
    internal void SetValueBehindLinkedValue(SQL_ColumnItem? column, SQL_RowItem? row, string value) {
        ...
        OnCellValueChanged(new SQL_CellEventArgs(column, row));
    }
    ...
    internal void SystemSet(SQL_ColumnItem? column, SQL_RowItem? row, string value) {
        ...
        if (ContainsKey(cellKey)) {
            ...
            Add(cellKey, new SQL_CellItem(string.Empty, 0, 0));
        }
    ...
    private static void MakeNewRelations(SQL_ColumnItem? column, SQL_RowItem? row, ICollection<string> oldBz, List<string> newBz) {
    ...
    private static (SQL_ColumnItem? column, SQL_RowItem? row, string info) RepairLinkedCellValue(SQL_Database linkedDatabase, SQL_ColumnItem column, SQL_RowItem? row, bool addRowIfNotExists) {
        SQL_RowItem? targetRow = null;
    ...
    private string ChangeTextFromRowId(string completeRelationText) {
        foreach (var SQL_RowItem in _database.Row) {
            if (SQL_RowItem != null) {
                completeRelationText = completeRelationText.Replace("/@X" + SQL_RowItem.Key + "X@/", SQL_RowItem.CellFirstString());
            }
        }
    ...
    private void ChangeValueOfKey(string currentvalue, SQL_ColumnItem? column, long rowKey) {
        ...
        var rows = keyc.Format == DataFormat.RelationText
            ...
            : SQL_RowCollection.MatchesTo(new SQL_FilterItem(keyc, FilterType.Istgleich_GroßKleinEgal, ownRow.CellGetString(keyc)));
        rows.Remove(ownRow);
    ...
    private void RelationTextNameChanged(SQL_ColumnItem? columnToRepair, long rowKey, string oldValue, string newValue) {
        if (string.IsNullOrEmpty(newValue)) { return; }
        foreach (var thisSQL_RowItem in _database.Row) {
            if (thisSQL_RowItem != null) {
                if (!thisSQL_RowItem.CellIsNullOrEmpty(columnToRepair)) {
                    var t = thisSQL_RowItem.CellGetString(columnToRepair);
                    if (!string.IsNullOrEmpty(oldValue) && t.ToUpper().Contains(oldValue.ToUpper())) {
                        ...
                        thisSQL_RowItem.CellSet(columnToRepair, t2);
                    }
                    if (t.ToUpper().Contains(newValue.ToUpper())) {
                        MakeNewRelations(columnToRepair, thisSQL_RowItem, new List<string>(), t.SplitAndCutByCrToList());
                    }
                }
            }
        }
    ...
    /// <param name="row"></param>
    /// <param name="previewsValue"></param>
    private void RepairRelationText(SQL_ColumnItem? column, SQL_RowItem? row, string previewsValue) {
    ...
    /// <param name="currentvalue"></param>
    private void SetSameValueOfKey(SQL_ColumnItem? column, SQL_RowItem ownRow, string currentvalue) {
        if (_database == null || _database.IsDisposed) { return; }
        if (column == null) { return; }
        List<SQL_RowItem>? rows = null;
            ...
            if (thisColumn.KeyColumnKey == column.Key) {
                if (rows == null) {
                    rows = column.Format == DataFormat.RelationText
                        ...
                        : SQL_RowCollection.MatchesTo(new SQL_FilterItem(column, FilterType.Istgleich_GroßKleinEgal, currentvalue));
                    rows.Remove(ownRow);
                }
                if (rows.Count < 1) {
