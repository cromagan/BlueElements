...
public sealed class SQL_ColumnItem : IReadableTextWithChanging, IDisposable, IInputFormat {
    ...
    private SQL_Database? _tmpLinkedDatabase;
    ...
    public SQL_Database? LinkedDatabase {
        get {
            ...
            Tmp_LinkedDatabase = _linkedDatabaseFile.Contains(@"\")
                ? SQL_Database.GetByFilename(_linkedDatabaseFile.FileNameWithoutSuffix(), false, Database._sql)
                : SQL_Database.GetByFilename(Database.TableName.FilePath() + _linkedDatabaseFile, false, Database._sql);
    ...
    private SQL_Database Tmp_LinkedDatabase {
        set {
    ...
    public static EditTypeTable UserEditDialogTypeInTable(SQL_ColumnItem? column, bool doDropDown) => UserEditDialogTypeInTable(column.Format, doDropDown, column.TextBearbeitungErlaubt, column.MultiLine);
    ...
    public void CloneFrom(SQL_ColumnItem source) {
    ...
    public List<string> Contents() => Contents(null as SQL_FilterCollection, null);
    ...
    /// <returns></returns>
    public List<string> Contents(List<SQL_FilterItem>? filter, List<SQL_RowItem>? pinned) {
        if (filter == null || filter.Count == 0) { return Contents(); }
        var ficol = new SQL_FilterCollection(filter[0].Database);
    ...
    /// <param name="pinned"></param>
    /// <returns></returns>
    public List<string> Contents(SQL_FilterItem filter, List<SQL_RowItem>? pinned) {
        var x = new SQL_FilterCollection(filter.Database) { filter };
        return Contents(x, pinned);
    }

    public List<string> Contents(List<SQL_RowItem?>? pinned) {
        List<string> list = new();

        if (pinned == null || pinned.Count == 0) { return list; }

        foreach (var thisSQL_RowItem in pinned) {
            if (thisSQL_RowItem != null) {
                if (_multiLine) {
                    list.AddRange(thisSQL_RowItem.CellGetList(this));
                } else {
                    if (thisSQL_RowItem.CellGetString(this).Length > 0) {
                        list.Add(thisSQL_RowItem.CellGetString(this));
                    }
                }
            }
        }
    ...
    /// <param name="pinned"></param>
    /// <returns></returns>
    public List<string> Contents(SQL_FilterCollection filter, List<SQL_RowItem>? pinned) {
        List<string> list = new();
        if (Database == null) { return list; }

        foreach (var thisSQL_RowItem in Database.Row) {
            if (thisSQL_RowItem != null) {
                var add = thisSQL_RowItem.MatchesTo(filter);
                if (!add && pinned != null) { add = pinned.Contains(thisSQL_RowItem); }
                if (add) {
                    if (_multiLine) {
                        list.AddRange(thisSQL_RowItem.CellGetList(this));
                    } else {
    ...
    /// <param name="pinned"></param>
    /// <returns></returns>
    public void DeleteContents(SQL_FilterCollection filter, List<SQL_RowItem>? pinned) {
        foreach (var thisSQL_RowItem in Database.Row) {
            if (thisSQL_RowItem != null) {
                if (thisSQL_RowItem.MatchesTo(filter) || pinned.Contains(thisSQL_RowItem)) { thisSQL_RowItem.CellSet(this, ""); }
            }
        }
    }
    ...
    public List<string> GetUcaseNamesSortedByLenght() {
        ...
        var tmp = Contents((SQL_FilterCollection)null, null);
    ...
    //    }
    //    return new SQL_ColumnItem(this, false);
    //}
    public void GetUniques(List<SQL_RowItem?> rows, out List<string> einzigartig, out List<string> nichtEinzigartig) {
    ...
    public void Invalidate_ColumAndContent() {
        ...
        foreach (var thisRow in Database.Row) {
            if (thisRow != null) { SQL_CellCollection.Invalidate_CellContentSize(this, thisRow); }
        }
    ...
    public SQL_ColumnItem? Previous() {
        var columnCount = Index();
        do {
            columnCount--;
    ...
    public string ReadableText() {
        var ret = _caption;
        if (Database.Column.Any(thisSQL_ColumnItem => thisSQL_ColumnItem != null && thisSQL_ColumnItem != this && string.Equals(thisSQL_ColumnItem.Caption, _caption, StringComparison.OrdinalIgnoreCase))) {
            var done = false;
    ...
    public void Statisik(List<SQL_FilterItem>? filter, List<SQL_RowItem?>? pinnedRows) {
    ...
    public double? Summe(SQL_FilterCollection? filter) {
        double summ = 0;
        foreach (var thisrow in Database.Row) {
    ...
    public double? Summe(List<SQL_RowData?> sort) {
        double summ = 0;
    ...
    private void _TMP_LinkedDatabase_Cell_CellValueChanged(object sender, SQL_CellEventArgs e) {
        ...
        foreach (var thisRow in Database.Row) {
            if (Database.Cell.GetStringBehindLinkedValue(this, thisRow) == tKey) {
                SQL_CellCollection.Invalidate_CellContentSize(this, thisRow);
                Invalidate_TmpColumnContentWidth();
                Database.Cell.OnCellValueChanged(new SQL_CellEventArgs(this, thisRow));
