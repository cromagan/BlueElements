...
public static class SQL_Export {
    ...
    //}
    public static string CreateLayout(SQL_RowItem row, string loadFile, string saveFile) {
        if (!FileExists(loadFile)) { return "Datei nicht gefunden."; }
        List<SQL_RowItem?> tmpList = new()
        {
            row
        };
    ...
    public static string CreateLayout(List<SQL_RowItem?> rows, string loadFile, string saveFile) {
        if (!FileExists(loadFile)) { return "Datei nicht gefunden."; }
    ...
    //    '   End If
    //End Sub
    public static (List<string>? files, string error) GenerateLayout_FileSystem(List<SQL_RowItem?>? liste, string lad, string optionalFileName, bool eineGrosseDatei, string zielPfad) {
        List<string> l = new();
    ...
    //    } while (true);
    //}
    public static (List<string>? files, string error) SaveAs(SQL_RowItem row, string layout, string destinationFile) {
    ...
    public static void SaveAsBitmap(SQL_RowItem row, string layoutId, string filename) => row.Database.OnGenerateLayoutInternal(new SQL_GenerateLayoutInternalEventargs(row, layoutId, filename));

    private static string InternalCreateLayout(List<SQL_RowItem?>? rows, string fileLoaded, string saveFileName) {
