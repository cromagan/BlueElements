...
public sealed class SQL_ExportDefinition : IParseable, IReadableTextWithChanging, IDisposable {
    ...
    private SQL_FilterCollection? _filter;
    ...
    public SQL_ExportDefinition(SQL_Database database, string verzeichnis, ExportTyp typ, float backupinterval, float autodelete) : this(database) {
    ...
    public SQL_ExportDefinition(SQL_Database database, string toParse) : this(database) {
        Parse(toParse);
    }

    public SQL_ExportDefinition(SQL_Database database) {
        Database = database;
        ...
        Filter = new SQL_FilterCollection(Database);
    ...
    public SQL_FilterCollection? Filter {
        get => _filter;
        set {
    ...
    public object Clone() => new SQL_ExportDefinition(Database, ToString());
        ...
        foreach (var pair in toParse.GetAllTags()) {
            switch (pair.Key) {
                ...
                case "filter": // ALT, 02.10.2019
                    Filter = new SQL_FilterCollection(Database, pair.Value);
                    break;
                            ...
                            if (_filter == null || _filter.Count < 1 || thisrow.MatchesTo(_filter)) {
                                ...
                                if (!found) {
                                    if (_exportFormularId.StartsWith("#")) {
                                        ...
                                        SQL_Export.SaveAsBitmap(thisrow, _exportFormularId, singleFileExport);
                                    } else {
                                        singleFileExport = TempFile(savePath, thisrow.CellFirstString().StarkeVereinfachung(" "), _exportFormularId.FileSuffix());
                                        SQL_Export.SaveAs(thisrow, _exportFormularId, singleFileExport);
                                    }
