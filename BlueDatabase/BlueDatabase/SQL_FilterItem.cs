...
public sealed class SQL_FilterItem : IParseable, IReadableTextWithChanging, ICanBeEmpty, IDisposable {

    #region Fields

    public string Herkunft = string.Empty;

    private SQL_ColumnItem? _column;
    ...
    public SQL_FilterItem(SQL_Database? database, FilterType filterType, string searchValue) : this(database, filterType, new List<string> { searchValue }) { }

    public SQL_FilterItem(SQL_Database? database, FilterType filterType, IReadOnlyCollection<string>? searchValue) {
        Database = database;
        if (Database != null) {
    ...
    public SQL_FilterItem(SQL_Database database, string filterCode) {
        Database = database;
        if (Database != null) {
            Database.Disposing += Database_Disposing;
        }
    ...
    public SQL_FilterItem(string filterCode) {
        Parse(filterCode);
        SearchValue.Changed += SearchValue_ListOrItemChanged;
    }

    public SQL_FilterItem(SQL_ColumnItem? column, FilterType filterType, string searchValue) : this(column, filterType, new List<string> { searchValue }, string.Empty) { }

    public SQL_FilterItem(SQL_ColumnItem? column, FilterType filterType, string searchValue, string tag) : this(column, filterType, new List<string> { searchValue }, tag) { }

    public SQL_FilterItem(SQL_ColumnItem? column, FilterType filterType, List<string> searchValue) : this(column, filterType, searchValue, string.Empty) { }

    public SQL_FilterItem(SQL_ColumnItem? column, FilterType filterType, IReadOnlyCollection<string>? searchValue, string herkunft) {
        if (column == null) {
    ...
    public SQL_FilterItem(SQL_ColumnItem? column, SQL_RowItem rowWithValue) : this(column, FilterType.Istgleich_GroﬂKleinEgal_MultiRowIgnorieren, rowWithValue.CellGetString(column)) { }
    ...
    //public object Clone() => new SQL_FilterItem(Database, ToString());
