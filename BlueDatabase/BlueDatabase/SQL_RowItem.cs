...
public sealed class SQL_RowItem : ICanBeEmpty, IDisposable {
    ...
    public SQL_RowItem(SQL_Database? database, long key) {
        Database = database;
        Key = key;
        _tmpQuickInfo = null;
        if (Database != null) {
    ...
    public SQL_RowItem(SQL_Database database) : this(database, database.Row.NextRowKey()) { }
    ...
    /// <returns></returns>
    public static List<Variable>? CellToVariable(SQL_ColumnItem? column, SQL_RowItem? row) {
    ...
    public bool CellGetBoolean(SQL_ColumnItem? column) => Database.Cell.GetBoolean(column, this);

    public Color CellGetColor(string columnName) => Database.Cell.GetColor(Database.Column[columnName], this);

    public Color CellGetColor(SQL_ColumnItem? column) => Database.Cell.GetColor(column, this);

    public int CellGetColorBgr(SQL_ColumnItem? column) => Database.Cell.GetColorBgr(column, this);

    public string CellGetCompareKey(SQL_ColumnItem? column) => Database.Cell.CompareKey(column, this);

    public DateTime CellGetDateTime(string columnName) => Database.Cell.GetDateTime(Database.Column[columnName], this);

    public DateTime CellGetDateTime(SQL_ColumnItem? column) => Database.Cell.GetDateTime(column, this);

    public double CellGetDouble(string columnName) => Database.Cell.GetDouble(Database.Column[columnName], this);

    public double CellGetDouble(SQL_ColumnItem? column) => Database.Cell.GetDouble(column, this);

    public int CellGetInteger(string columnName) => Database.Cell.GetInteger(Database.Column[columnName], this);

    public int CellGetInteger(SQL_ColumnItem? column) => Database.Cell.GetInteger(column, this);

    public List<string> CellGetList(string columnName) => Database.Cell.GetList(Database.Column[columnName], this);

    public List<string> CellGetList(SQL_ColumnItem? column) => Database.Cell.GetList(column, this);

    public Point CellGetPoint(string columnName) => Database.Cell.GetPoint(Database.Column[columnName], this);

    public Point CellGetPoint(SQL_ColumnItem? column) => Database.Cell.GetPoint(column, this);

    public string CellGetString(string columnName) => Database.Cell.GetString(Database.Column[columnName], this);

    public string CellGetString(SQL_ColumnItem? column) => Database.Cell.GetString(column, this);

    public List<string> CellGetValuesReadable(SQL_ColumnItem? column, ShortenStyle style) => Database.Cell.ValuesReadable(column, this, style);

    public bool CellIsNullOrEmpty(string columnName) => Database.Cell.IsNullOrEmpty(Database.Column[columnName], this);

    public bool CellIsNullOrEmpty(SQL_ColumnItem? column) => Database.Cell.IsNullOrEmpty(column, this);

    public void CellSet(string columnName, bool value) => Database.Cell.Set(Database.Column[columnName], this, value);

    public void CellSet(SQL_ColumnItem? column, bool value) => Database.Cell.Set(column, this, value);

    public void CellSet(string columnName, string value) => Database.Cell.Set(Database.Column[columnName], this, value);

    public void CellSet(SQL_ColumnItem? column, string value) => Database.Cell.Set(column, this, value);

    public void CellSet(string columnName, double value) => Database.Cell.Set(Database.Column[columnName], this, value);

    public void CellSet(SQL_ColumnItem? column, double value) => Database.Cell.Set(column, this, value);

    public void CellSet(string columnName, int value) => Database.Cell.Set(Database.Column[columnName], this, value);

    public void CellSet(SQL_ColumnItem? column, int value) => Database.Cell.Set(column, this, value);

    public void CellSet(string columnName, Point value) => Database.Cell.Set(Database.Column[columnName], this, value);

    public void CellSet(SQL_ColumnItem? column, Point value) => Database.Cell.Set(column, this, value);

    public void CellSet(string columnName, List<string>? value) => Database.Cell.Set(Database.Column[columnName], this, value);

    public void CellSet(SQL_ColumnItem? column, List<string>? value) => Database.Cell.Set(column, this, value);

    public void CellSet(string columnName, DateTime value) => Database.Cell.Set(Database.Column[columnName], this, value);

    public void CellSet(SQL_ColumnItem? column, DateTime value) => Database.Cell.Set(column, this, value);
    ...
    /// <param name="columns">Nur diese Spalten in deser Reihenfolge werden berücksichtigt</param>
    /// <returns>Den String mit dem abschluß <<>key<>> und dessen Key.</returns>
    public string CompareKey(List<SQL_ColumnItem>? columns) {
        StringBuilder r = new();
        if (columns != null) {
    ...
    public (bool checkPerformed, string error, Script? skript) DoAutomatic(bool doFemdZelleInvalidate, bool fullCheck, string startroutine) {
        ...
        if (!string.IsNullOrEmpty(script.Error)) {
            Database.OnScriptError(new SQL_RowCancelEventArgs(this, "Zeile: " + script.Line + "\r\n" + script.Error + "\r\n" + script.ErrorCode));
        ...
        foreach (var thisColum in Database.Column.Where(thisColum => thisColum != null)) {
            if (fullCheck) {
                ...
                SQL_CellCollection.Invalidate_CellContentSize(thisColum, this);
        ...
        OnRowChecked(new SQL_RowCheckedEventArgs(this, cols));
        return (true, infoTxt, script);
    }

    public bool IsNullOrEmpty() => Database.Column.All(thisSQL_ColumnItem => thisSQL_ColumnItem != null && CellIsNullOrEmpty(thisSQL_ColumnItem));

    public bool IsNullOrEmpty(SQL_ColumnItem? column) => Database.Cell.IsNullOrEmpty(column, this);

    public bool IsNullOrEmpty(string columnName) => Database.Cell.IsNullOrEmpty(Database.Column[columnName], this);

    public bool MatchesTo(SQL_FilterItem? filter) {
    ...
    public bool MatchesTo(List<SQL_FilterItem>? filter) {
        if (Database == null) { return false; }
    ...
    public string ReplaceVariables(string formel, bool fulltext, bool removeLineBreaks) {
        ...
        var erg = formel;
        // Variablen ersetzen
        foreach (var thisSQL_ColumnItem in Database.Column) {
            if (!erg.Contains("~")) { return erg; }
            if (thisSQL_ColumnItem != null) {
                var txt = CellGetString(thisSQL_ColumnItem);
                if (fulltext) { txt = SQL_CellItem.ValueReadable(thisSQL_ColumnItem, txt, ShortenStyle.Replaced, BildTextVerhalten.Nur_Text, removeLineBreaks); }
                if (removeLineBreaks && !fulltext) {
                    txt = txt.Replace("\r\n", " ");
                    txt = txt.Replace("\r", " ");
                }

                erg = erg.Replace("~" + thisSQL_ColumnItem.Name.ToUpper() + "~", txt, RegexOptions.IgnoreCase);
                while (erg.ToUpper().Contains("~" + thisSQL_ColumnItem.Name.ToUpper() + "(")) {
                    var x = erg.ToUpper().IndexOf("~" + thisSQL_ColumnItem.Name.ToUpper() + "(");
                    var x2 = erg.IndexOf(")", x);
                    if (x2 < x) { return erg; }
                    var ww = erg.Substring(x + thisSQL_ColumnItem.Name.Length + 2, x2 - x - thisSQL_ColumnItem.Name.Length - 2);
    ...
    internal void OnDoSpecialRules(SQL_DoRowAutomaticEventArgs e) => DoSpecialRules?.Invoke(this, e);

    internal void OnRowChecked(SQL_RowCheckedEventArgs e) => RowChecked?.Invoke(this, e);

    private void Cell_CellValueChanged(object sender, SQL_CellEventArgs e) {
    ...
    private Script DoRules(string startRoutine) {
        try {
            ...
            if (startRoutine != "script testing" && ((VariableBool)vars.GetSystem("CellChangesEnabled")).ValueBool) {
                ...
                SQL_DoRowAutomaticEventArgs e = new(this);
                OnDoSpecialRules(e);
    ...
    private bool RowFilterMatch(string searchText) {
        ...
        foreach (var thisSQL_ColumnItem in Database.Column) {
            {
                if (!thisSQL_ColumnItem.IgnoreAtRowFilter) {
                    var @string = CellGetString(thisSQL_ColumnItem);
                    @string = LanguageTool.ColumnReplace(@string, thisSQL_ColumnItem, ShortenStyle.Both);
    ...
    private void VariableToCell(SQL_ColumnItem? column, List<Variable> vars) {
