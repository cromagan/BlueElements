...
public sealed class SQL_RowSortDefinition : IParseable {

    #region Fields

    public readonly ListExt<SQL_ColumnItem> Columns = new();
    ...
    public SQL_RowSortDefinition(SQL_Database database, string code) {
        Database = database;
        Parse(code);
    }

    public SQL_RowSortDefinition(SQL_Database database, string columnName, bool reverse) {
        Database = database;
        Initialize();
        Reverse = reverse;
    ...
    public SQL_RowSortDefinition(SQL_Database database, List<string> columnNames, bool reverse) {
        Initialize();
        Database = database;
        Reverse = reverse;
        ...
        foreach (var pair in toParse.GetAllTags()) {
            switch (pair.Key) {
                ...
                case "columnname": // Columname wichtig wegen CopyLayout
                    if (Database.Column[pair.Value] is SQL_ColumnItem c) { Columns.Add(c); }

                    break;

                case "columnkey":
                    if (Database.Column.SearchByKey(LongParse(pair.Value)) is SQL_ColumnItem c2) { Columns.Add(c2); }
                    break;
    ...
    public bool UsedForRowSort(SQL_ColumnItem? vcolumn) {
