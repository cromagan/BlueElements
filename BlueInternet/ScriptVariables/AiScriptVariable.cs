...
public class AiScriptVariable : ScriptVariable {
    ...
    public AiScriptVariable(string name, string? apiKey, string? endpoint, string? model, bool ronly, string comment) : base(name, ronly, comment) {
        _apiKey = apiKey;
        _endpoint = endpoint;
        _model = model;
    }
    ...
    /// </summary>
    public AiScriptVariable(string name) : this(name, null, null, null, true, string.Empty) { }

    public AiScriptVariable(string? apiKey, string? endpoint, string? model) : this(DummyName(), apiKey, endpoint, model, true, string.Empty) { }
    ...
    /// </summary>
    public static async Task<string?> AskAsync(string? apiKey, string? endpoint, string? model, string prompt, System.Drawing.Bitmap? image) {
        ...
        if (image is null) {
            ...
            var base64 = BitmapToBase64(image, ImageFormat.Jpeg);
            body["messages"] = new JsonArray {
                new JsonObject {
                    ["role"] = "user",
    ...
    /// sondern gibt null zurück.
    /// </summary>
    public static async Task<System.Drawing.Bitmap?> GenerateImageAsync(string? apiKey, string? endpoint, string imageModel, string prompt) {
        ...
        try {
            ...
            if (root?["data"]?[0]?["b64_json"] is { } b64) {
                return Base64ToBitmap(b64.ToString());
            }

            if (root?["data"]?[0]?["url"] is { } url) {
                var img = DownloadImage(url.ToString());
                return img is null ? null : new System.Drawing.Bitmap(img);
            }
    ...
    public override string GetValueFrom(ScriptVariable variable) {
        if (variable is not AiScriptVariable v) { return VerschiedeneTypen(variable); }
