...
namespace BluePaint {
    public partial class Tool_Spiegeln : GenericTool // System.Windows.Forms.UserControl // 
    {
        ...
        public override void MouseUp(BlueControls.EventArgs.MouseEventArgs1_1DownAndCurrent e, Bitmap OriginalPic) {
            ...
            float Wink = (float)GeometryDF.Winkel(new PointM(e.MouseDown.X, e.MouseDown.Y), new PointM(e.Current.X, e.Current.Y));
            ...
            float MinX = Math.Min(Math.Min(p[0].X, p[1].X), Math.Min(p[2].X, p[3].X));
            float MinY = Math.Min(Math.Min(p[0].Y, p[1].Y), Math.Min(p[2].Y, p[3].Y));
            float MaxX = Math.Max(Math.Max(p[0].X, p[1].X), Math.Max(p[2].X, p[3].X));
            float MaxY = Math.Max(Math.Max(p[0].Y, p[1].Y), Math.Max(p[2].Y, p[3].Y));

            int B = (int)Math.Round(MaxX - MinX);
            int H = (int)Math.Round(MaxY - MinY);
            Bitmap nBMP = new Bitmap(B, H);
            ...
            using (Graphics gr = Graphics.FromImage(nBMP)) {
                gr.InterpolationMode = InterpolationMode.HighQualityBicubic;
                gr.Clear(Color.Magenta);
                gr.Transform = rotate_at_center;

                // Draw the image centered on the bitmap.
                int x = (B - _Pic.Width) / 2;
                int y = (H - _Pic.Height) / 2;
