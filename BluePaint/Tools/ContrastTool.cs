// Licensed under AGPL-3.0; see License.md for disclaimer and details.

using BlueBasics.ImageFilters;
using BlueControls.EventArgs;

namespace BluePaint;

public partial class ContrastTool : Tool //System.Windows.Forms.UserControl //
{
    #region Constructors

    public ContrastTool() : base() => InitializeComponent();

    #endregion

    #region Methods

    public override void DrawOverlay(Graphics gr, float zoom, int offsetX, int offsetY, TrimmedCanvasMouseEventArgs? mouseDown, TrimmedCanvasMouseEventArgs? mouseCurrent) {
        var originalPic = OnNeedCurrentPic();
        if (originalPic?.IsValid() != true) { return; }

        using var picPreview = originalPic.CloneFromBitmap();

        var filters = new List<(ImageFilter filter, object? parameter)>();
        if (sldKontrast.Value != 0) { filters.Add((ContrastImageFilter.Instance, sldKontrast.Value)); }
        if (Math.Abs(sldGamma.Value - 1) > 0.001) { filters.Add((GammaImageFilter.Instance, sldGamma.Value)); }
        if (Math.Abs(sldHelligkeit.Value - 1) > 0.001) { filters.Add((BrightnessImageFilter.Instance, sldHelligkeit.Value)); }
        picPreview.ApplyFilter(filters.ToArray());

        var ctrlRect = new RectangleF(0, 0, picPreview.Width, picPreview.Height).CanvasToControl(zoom, offsetX, offsetY, false);
        gr.DrawImage(picPreview, ctrlRect.X, ctrlRect.Y, ctrlRect.Width, (float)ctrlRect.Height);
    }

    private void btnAlleFarbenSchwarz_Click(object sender, EventArgs e) {
        var pic = OnNeedCurrentPic();
        if (pic is null) { return; }
        var picPreview = pic.CloneFromBitmap();

        picPreview.ApplyFilter((AllePixelZuSchwarzImageFilter.Instance, 0.95f));

        OnOverridePic(picPreview, false);
        sldGamma.Value = 1f;
        sldKontrast.Value = 0f;
        sldHelligkeit.Value = 1f;
    }

    private void btnAusdünnen_Click(object sender, EventArgs e) {
        var pic = OnNeedCurrentPic();
        if (pic is null) { return; }
        var picPreview = pic.CloneFromBitmap();

        picPreview.ApplyFilter((AusdünnenImageFilter.Instance, 4));
        OnOverridePic(picPreview, false);
        sldGamma.Value = 1f;
        sldKontrast.Value = 0f;
        sldHelligkeit.Value = 1f;
    }

    private void btnGamma_Click(object sender, EventArgs e) => DoPic();

    private void btnGraustufen_Click(object sender, EventArgs e) {
        var pic = OnNeedCurrentPic();
        if (pic is null) { return; }
        var picPreview = pic.CloneFromBitmap();

        picPreview.ApplyFilter(GrayscaleImageFilter.Instance);

        OnOverridePic(picPreview, false);
        sldGamma.Value = 1f;
        sldKontrast.Value = 0f;
        sldHelligkeit.Value = 1f;
    }

    private void btnHelligkeit_Click(object sender, EventArgs e) => DoPic();

    private void btnKontrastErhoehen_Click(object sender, EventArgs e) => DoPic();

    private void btnPixelHinzu_Click(object sender, EventArgs e) {
        var pic = OnNeedCurrentPic();
        if (pic is null) { return; }
        var picPreview = pic.CloneFromBitmap();

        picPreview.ApplyFilter(SchwarzePixelHinzufügenImageFilter.Instance);

        OnOverridePic(picPreview, false);
        sldGamma.Value = 1f;
        sldKontrast.Value = 0f;
        sldHelligkeit.Value = 1f;
    }

    private void DoPic() {
        var pic = OnNeedCurrentPic();
        if (pic is null) { return; }

        var picPreview = pic.CloneFromBitmap();

        var filters = new List<(ImageFilter filter, object? parameter)>();
        if (sldKontrast.Value != 0) { filters.Add((ContrastImageFilter.Instance, sldKontrast.Value)); }
        if (Math.Abs(sldGamma.Value - 1) > 0.001) { filters.Add((GammaImageFilter.Instance, sldGamma.Value)); }
        if (Math.Abs(sldHelligkeit.Value - 1) > 0.001) { filters.Add((BrightnessImageFilter.Instance, sldHelligkeit.Value)); }
        picPreview.ApplyFilter(filters.ToArray());

        OnOverridePic(picPreview, false);
        sldGamma.Value = 1f;
        sldKontrast.Value = 0f;
        sldHelligkeit.Value = 1f;
    }

    private void sldGamma_ValueChanged(object sender, EventArgs e) {
        capGamma.Text = sldGamma.Value.ToString1_2();
        OnDoInvalidate();
    }

    private void sldHelligkeit_ValueChanged(object sender, EventArgs e) {
        capHelligkeit.Text = sldHelligkeit.Value.ToString1_2();
        OnDoInvalidate();
    }

    private void sldKontrast_ValueChanged(object sender, EventArgs e) {
        capKontrast.Text = sldKontrast.Value.ToString1_2();
        OnDoInvalidate();
    }

    #endregion
}