...
internal class Method_StringToUTF8 : Method {
    ...
    public override List<string>Comand(List<Variable>? currentvariables) => new() { "stringtoutf8" };

    public override DoItFeedback DoIt(Script s, CanDoFeedback infos) {
        var attvar = SplitAttributeToVars(s, infos.AttributText, Args, EndlessArgs);
        return !string.IsNullOrEmpty(attvar.ErrorMessage) ? DoItFeedback.AttributFehler(s, infos, this, attvar)
            : new DoItFeedback(s, infos, ((VariableString)attvar.Attributes[0]).ValueString.StringtoUtf8(), string.Empty);
    }
