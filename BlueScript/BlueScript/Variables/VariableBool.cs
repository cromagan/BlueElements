...
public class VariableBool : Variable {
    ...
    public override DoItFeedback GetValueFrom(Variable variable) {
        if (variable is not VariableBool v) { return DoItFeedback.VerschiedeneTypen(null, null, this, variable); }
    ...
    protected override object? TryParse(string txt, Script? s) {
        ...
        if (i > -1) {
            if (i < 1 && check != "!") {
                return false;//new DoItFeedback(infos, s, "Operator (" + check + ") am String-Start nicht erlaubt: " + txt);
            } // <1, weil ja mindestens ein Zeichen vorher sein MUSS!

            if (i >= txt.Length - 1) {
                return false;//new DoItFeedback(infos, s, "Operator (" + check + ") am String-Ende nicht erlaubt: " + txt);
            } // siehe oben
            ...
            if (!string.IsNullOrEmpty(s1)) {
                ...
                if (!string.IsNullOrEmpty(tmp1.ErrorMessage)) { return false; }//new DoItFeedback(infos, s, "Befehls-Berechnungsfehler in ():" + tmp1.ErrorMessage);

                v1 = tmp1.Variable;
            } else {
                if (check != "!") { return false; }//new DoItFeedback(infos, s, "Wert vor Operator (" + check + ") nicht gefunden: " + txt);
            }
            ...
            if (string.IsNullOrEmpty(s2)) { return false; }//new DoItFeedback(infos, s, "Wert nach Operator (" + check + ") nicht gefunden: " + txt);

            var tmp2 = GetVariableByParsing(s2, s);
            if (!string.IsNullOrEmpty(tmp2.ErrorMessage)) {
                return false;//new DoItFeedback(infos, s, "Befehls-Berechnungsfehler in ():" + tmp1.ErrorMessage);
            }
            ...
            if (v1 != null) {
                if (v1.MyClassId != v2.MyClassId) { return false; }// return new DoItFeedback(infos, s, "Typen unterschiedlich: " + txt);

                if (!v1.ToStringPossible) { return false; }// return new DoItFeedback(infos, s, "Datentyp nicht zum Vergleichen geeignet: " + txt);
            } else {
                if (v2 is not VariableBool) { return false; }// return new DoItFeedback(infos, s, "Datentyp nicht zum Vergleichen geeignet: " + txt);
            }
            ...
            switch (check) {
                ...
                case ">=": {
                        if (v1 is not VariableFloat v1Fl) { return false; } //  return new DoItFeedback(infos, s, "Datentyp nicht zum Vergleichen geeignet: " + txt);
                        if (v2 is not VariableFloat v2Fl) { return false; }  //  return new DoItFeedback(infos, s, "Datentyp nicht zum Vergleichen geeignet: " + txt);
                ...
                case "||": {
                        if (v1 is not VariableBool v1Bo) { return false; }                            // return new DoItFeedback(infos, s, "Datentyp nicht zum Vergleichen geeignet: " + txt);
                        if (v2 is not VariableBool v2Bo) { return false; }// return new DoItFeedback(infos, s, "Datentyp nicht zum Vergleichen geeignet: " + txt);
                ...
                default:
                    return false; //  return new DoItFeedback(infos, s, "Operator (" + check + ") unbekannt: " + txt);
            }
