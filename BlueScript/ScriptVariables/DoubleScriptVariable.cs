...
public class DoubleScriptVariable : ScriptVariable {
    ...
    public DoubleScriptVariable(string name, double value, bool ronly, string comment) : base(name, ronly, comment) => _double = value;

    public DoubleScriptVariable(double value) : this(DummyName(), value, true, string.Empty) { }

    public DoubleScriptVariable() : this(string.Empty, 0f, true, string.Empty) { }

    public DoubleScriptVariable(string name) : this(name, 0f, true, string.Empty) { }
    ...
    public override string ValueForCell {
        get => ReadableText;
        set => ValueNum = DoubleParse(value);
    }
    ...
    public override string GetValueFrom(ScriptVariable variable) {
        if (variable is not DoubleScriptVariable v) { return VerschiedeneTypen(variable); }
