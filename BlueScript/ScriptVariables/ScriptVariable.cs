...
public abstract class ScriptVariable : ParseableItem, IComparable, IParseable, IHasKeyName, IJsonParseable {

    #region Fields

    public static readonly AssemblyAwareCache<ScriptVariable> VarTypes = new();

    private static readonly ConcurrentCache<Type, ScriptVariable> _instanceCache = new(30);
    ...
    protected ScriptVariable(string name, bool ronly, string comment) : base() {
        if (string.IsNullOrEmpty(name)) { name = GetUniqueKey(); }
    ...
    public virtual string ValueForCell {
        ...
        set => Develop.DebugError("Unbekannter Typ");
    }
    ...
    public static bool IsValidName(string v) {
        ...
        _commandNames ??= new HashSet<string>(ScriptCommand.AllMethods.Instances.Select(m => m.Command), StringComparer.OrdinalIgnoreCase);
        return !_commandNames.Contains(v);
    }

    public static bool TryParseValue<T>(string txt, out object? result) where T : ScriptVariable, new() {
    ...
    public bool TryParse(string txt, out ScriptVariable? succesVar) {
        ...
        var newVar = NewByTypeName<ScriptVariable>(MyClassId);
        if (newVar is null) {
            succesVar = null;
            return false;
        }
    ...
    public string VerschiedeneTypen(ScriptVariable var2) => $"Variable '{KeyName}' ist nicht der erwartete Typ {var2.MyClassId}, sondern {MyClassId}";
