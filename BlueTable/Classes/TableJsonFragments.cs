...
public class TableJsonFragments : TableJsonFile {
    ...
    /// <summary>
    /// Gibt das Suffix für JSON-Fragmentdateien zurück.
    /// </summary>
    public static readonly string SuffixOfJsonFragments = "frj";
    ...
    public TableJsonFragments(string filename, Table? source) : base(filename, source) { }
    ...
    public override bool BeSureToBeUpToDate(bool firstTime) {
        ...
        if (!IsDisposed && DropMessages) { Develop.Message(ErrorType.Info, this, Caption, ImageCode.Tabelle, "Lade JSON-Fragmente von '" + KeyName + "'", 0); }
    ...
    /// Schreibt abschließend ein JSON-EOF-Marker.
    /// </summary>
    private void CloseWriter() {
        ...
        if (writerToClose is not null) {
            ...
            try {
                var eof = new JsonObject { ["_meta"] = "eof" };
                writerToClose.WriteLine(eof.ToJsonString());
                writerToClose.Flush();
            } catch { } finally {
                try {
    ...
    /// <summary>
    /// Führt Aufräumarbeiten nach dem Laden von Fragmenten durch, inkl. Erstellung einer neuen JSON-Hauptdatei.
    /// </summary>
    private void DoWorkAfterLastChanges(List<string>? files, DateTime startTimeUtc) {
        ...
        if (_masterNeeded && AmITemporaryMaster(MasterTry, MasterUntil, true)) {
            Develop.Message(ErrorType.Info, this, Caption, ImageCode.Tabelle, "Erstelle neue JSON-Komplett-Tabelle: " + KeyName, 0);

            // Writer vorher schließen, damit die eigene Fragment-Datei nicht
            // gleichzeitig zur Komplettierung offen ist.
            CloseWriter();

            // Komplettierung über JSON-Save der Basisklasse (TableJsonFile.SaveInternal).
            // base.SaveInternal schreibt die komplette Tabelle als JSON in die Hauptdatei.
            var f = base.SaveInternal();
            ...
            if (f.Length > 19) {
                ...
                if (DateTimeTryParse(da, out var d2)) {
                    if (DateTime.UtcNow.Subtract(d2).TotalMinutes > DeleteFragmentsAfter &&
                        ...
                        Develop.Message(ErrorType.Info, this, Caption, ImageCode.Tabelle, "Räume JSON-Fragmente auf: " + thisf.FileNameWithoutSuffix(), 0);
        ...
        try {
            ...
            foreach (var thisf in frgma) {
                ...
                foreach (var thist in fil.SplitAndCutByCr()) {
                    if (string.IsNullOrWhiteSpace(thist)) { continue; }
                    if (thist[0] != '{') { continue; }

                    JsonObject? jo;
                    try {
                        jo = JsonNode.Parse(thist)?.AsObject();
                    } catch {
                        continue;
                    }

                    if (jo is null) { continue; }
    ...
    /// <summary>
    /// Initialisiert den StreamWriter für die JSON-Fragmentdatei.
    /// Schreibt einen NDJSON-Header mit Metadaten (Version, Filename, User).
    /// </summary>
    private void StartWriter() {
        ...
        var tmp = IO.TempFile(FragmengtsPath(), KeyName + "-" + Environment.MachineName + "-" + DateTime.UtcNow.ToString4(), SuffixOfJsonFragments);
        ...
        try {
            ...
            // Header als NDJSON-Steuerzeile schreiben.
            // Der Key "_meta" kennzeichnet Steuerzeilen; UndoItems haben diesen Key nicht.
